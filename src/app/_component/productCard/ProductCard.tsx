...
import { FaPlus, FaRegEye, FaRegStar, FaStar } from "react-icons/fa";
...
export default function ProductCard({ product }: Props) {
  const discountPercent: number = Math.round(
    ((product.price - (product.priceAfterDiscount ?? product.price)) / product.price) * 100,
  );
  return (
    <div className="bg-[#FFFFFF] rounded-lg overflow-hidden  border border-[#E5E7EB] col-span-1 transition-all duration-500 hover:-translate-y-1.5 shadow-black/20 hover:shadow-lg ">
      <div className="overflow-hidden relative h-63">
        ...
        <div className="absolute top-3 right-3">
          ...
          <Link href={`/products/${product.id}`} className="w-8 h-8  rounded-full bg-[#FFFFFF] shadow-[0_1px_2px_-1px_rgba(0,0,0,0.1),0_1px_3px_0_rgba(0,0,0,0.1)] flex items-center justify-center cursor-pointer transition-all duration-500 hover:text-main-color ">
            <FaRegEye />
          </Link>
        </div>
        {
      ...
      <div className="flex flex-col gap-1 p-4">
        ...
        <Link href={`/products/${product.id}`} className="text-[#364153] font-medium leading-6 text-[16px] line-clamp-1 ">
          {product.title}
        </Link>
        ...
        <div className="flex justify-between">
          ...
          <AddToCartButton id={product.id} className="w-10 h-10 text-white rounded-full bg-main-color flex items-center justify-center transition-all duration-500 hover:bg-main-color-hover cursor-pointer">
              <FaPlus color="white" />
          </AddToCartButton>
        </div>
      </div>
