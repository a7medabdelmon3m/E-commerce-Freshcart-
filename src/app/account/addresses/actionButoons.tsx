"use client";
import { deleteAddresseItem } from "@/api/actions/routea.ctions";
      ...
      if (result.isConfirmed)
        try {
          startTransition(async () => {
            const response = await deleteAddresseItem(addresseId);
    ...
    <div>
      <div className="flex  gap-2">
        <Button
          ...
          className="w-9 h-9 rounded-lg flex items-center justify-center bg-[#F3F4F6] text-[#4A5565] hover:bg-main-color-subtle hover:text-main-color transition-colors duration-100"
        >
          <FaPen />
        </Button>
        <Button
          onClick={handleDeleteAddress}
          className="w-9 h-9 rounded-lg flex items-center justify-center bg-[#F3F4F6] text-[#4A5565] hover:bg-red-100 hover:text-red-600 transition-colors duration-100"
        >
          <FaTrash />
        </Button>
        <AddAndEditModal
